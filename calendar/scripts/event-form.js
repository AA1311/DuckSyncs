...
export function initEventForm(toaster) {
  ...
  let mode = "create";
  let currentEventId = null; // Track the event ID separately for edits
  ...
  formElement.addEventListener("submit", (event) => {
    ...
    if (mode === "edit") {
      if (!currentEventId) {
        console.error("No valid event ID for edit mode");
        return;
      }
      formEvent.id = currentEventId; // Preserve the original ID
      console.log("Dispatching event-edit:", formEvent);
      formElement.dispatchEvent(new CustomEvent("event-edit", {
        detail: {
          event: formEvent
  ...
  return {
    ...
    switchToCreateMode(date, startTime, endTime) {
      mode = "create";
      currentEventId = null; // Reset ID for new events
      fillFormWithDate(formElement, date, startTime, endTime);
    },
    switchToEditMode(event) {
      mode = "edit";
      currentEventId = event.id; // Store the original ID
      if (!currentEventId) {
        console.error("Event ID missing in edit mode:", event);
      }
      fillFormWithEvent(formElement, event);
    },
    reset() {
...
function fillFormWithEvent(formElement, event) {
  ...
  idInputElement.value = event.id || ""; // Ensure ID is set in the form
  titleInputElement.value = event.title || "";
  dateInputElement.value = event.date.toISOString().substr(0, 10);
  startTimeSelectElement.value = event.startTime;
  endTimeSelectElement.value = event.endTime;
...
function formIntoEvent(formElement) {
  const formData = new FormData(formElement);
  const id = formData.get("id") || null; // Use form ID if present, otherwise null
  const title = formData.get("title");
  const date = formData.get("date");
  const startTime = formData.get("start-time");
  const endTime = formData.get("end-time");
  ...
  const event = {
    id, // Will be overridden in edit mode by currentEventId
    title,
    date: new Date(date),
    startTime: Number.parseInt(startTime, 10),
    endTime: Number.parseInt(endTime, 10),
